"""run_pet.py — 轻量桌宠启动器（无控制台）。

只负责：找到同目录 venv 下的无控制台 Python（pythonw），执行 main.py。
自身不导入任何重型依赖，打包后仅 ~7MB。通过 pythonw 运行，不弹出黑色命令行窗口；
启动后启动器立即退出，桌宠作为独立进程继续运行（关闭启动器不影响桌宠）。
"""
import ctypes
import os
import subprocess
...
def main():
    ...
    if not target.exists():
        sys.stderr.write(f"错误：找不到 main.py（搜索路径：{target}）\n")
        sys.exit(1)

    # pythonw 无控制台，把输出重定向到日志文件便于排查
    logf = None
    if sys.platform == "win32":
        try:
            logf = open(script_dir / "pet_debug.log", "a", encoding="utf-8")
        except Exception:
            logf = None
